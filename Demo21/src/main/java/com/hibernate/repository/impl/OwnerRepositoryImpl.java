...
public class OwnerRepositoryImpl implements OwnerRepository {
	...
	// SELECT o FROM Owner o JOIN FETCH o.pet
	...
	// SELECT o FROM Owner o JOIN FETCH o.pet WHERE o.firstName LIKE CONCAT(:firstName,'%')
	...
	public List<Owner> findAllOwnersByPetDateOfBirthRange(LocalDate startDate, LocalDate endDate) {
		try (Session session = sessionFactory.openSession()) {
		    ...
		    Root<Owner> root = criteriaQuery.from(Owner.class);
		    root.fetch("pet");
		    // ✅ Use treat here so that join can access birthDate (since it's in DomesticPet, not Pet)
		    Join<Owner, DomesticPet> join = hibernateCriteriaBuilder.treat(root.join("pet"), DomesticPet.class);
		    Predicate where = hibernateCriteriaBuilder.conjunction();
		    where = hibernateCriteriaBuilder.and(where,
	...
	// SELECT o FROM Owner o JOIN FETCH o.pet p WHERE p.birthDate BETWEEN :startDate AND :endDate ORDER BY p.birthDate
	...
	public List<Object[]> findIdAndFirstNameAndLastNameAndPetName(int pageNumber, int pageSize) {
		try (Session session = sessionFactory.openSession()) {
			...
			Selection<Object[]> selection = hibernateCriteriaBuilder
					                        .array(
					                        	root.get("id"),
											root.get("firstName"), 
											root.get("lastName"),
	...
	// SELECT o.id, o.firstName, o.lastName, p.name FROM Owner o JOIN o.pet p
}
